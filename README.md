...
| Tool | Does | Status |
|---|---|---|
| **`sweep`** | Organises the obvious and leaves the private alone | v0.4 |
| **`stash`** | Clears a folder now, decides nothing, brings it all back | v0.4 |
| **`unpack`** | One command for every archive format, safely | v0.4 |

> **On `main`, not yet released.** The project guard, the grace window and
> `--since` described below landed after v0.4.0 and ship in v0.5.0. Installing
> with `--tag v0.4.0` gets you a sweep that will happily reorganise a Godot
> project. Track `main` if you want the guard today.
