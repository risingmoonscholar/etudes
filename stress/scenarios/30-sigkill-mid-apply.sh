...
keep_evidence() {
  ...
  if [ -n "${ETUDE_STATE_DIR:-}" ] && [ -d "$ETUDE_STATE_DIR" ]; then
    mkdir -p "$dest/state-after-undo"
    newest=$(ls -t "$ETUDE_STATE_DIR"/*.journal 2>/dev/null | head -1)
    [ -n "$newest" ] && cp "$newest" "$dest/state-after-undo/" 2>/dev/null
  fi
...
trial() {
  ...
  # Copy the journal BEFORE undo, because undo rewrites it as it reverses.
  # Keeping it afterwards preserves undo's edits, not the state the kill left
  # behind -- which is the state that explains a stranding.
  # The NEWEST journal only, not the whole state dir. ETUDE_STATE_DIR
  # accumulates one journal per trial, so by trial 50 a full copy is 50
  # journals of which 49 belong to trials that passed -- 16 MB to keep 330 KB
  # of evidence. undo picks the newest by modification time, so the newest is
  # the one this trial is about.
  local pre_undo; pre_undo=$(mktemp -d)
  if [ -n "${ETUDE_STATE_DIR:-}" ] && [ -d "$ETUDE_STATE_DIR" ]; then
    local newest
    newest=$(ls -t "$ETUDE_STATE_DIR"/*.journal 2>/dev/null | head -1)
    [ -n "$newest" ] && cp "$newest" "$pre_undo/" 2>/dev/null
  fi
